 ...
 * 2016年11月1日 15:25:41
 */
@Service("procNewApplPSInfoService")
public class ProcNewApplPSInfoServiceImpl implements ProcNewApplPSInfoService {

	/**
	 * 日志管理工具实例.
	 */
	private  Logger logger = LoggerFactory.getLogger(ProcNewApplPSInfoServiceImpl.class);
	...
	public boolean procPSInfo(String applNo) {
		logger.info("开始执行账户余额冻结功能："+applNo);
		...
		if(PREM_SOURCE.GRP_ACC_PAY.getKey().equals(premSource)){	
	        ...
	        if(isFund){ //若是基金险，在直接取投保要约总保费
	        	this.logger.info("【保费来源：团体账户付款】基金险保费计算【"+applNo+"】！");
				String key=applNo+"O0";
				bigMap.put(key, grpInsurAppl.getApplState().getSumPremium());
	        }else{
		...
		//2、修改对应的缴费帐号
		return createEaccData(applNo,bigMap);
	}
	...
	private double getSumGrpPayAmount(String applNo) {
		...
		map.put("procStat", "E");
		List<GrpInsured> list = mongoBaseDao.find(GrpInsured.class,map);
		double sum=0;
	...
	private Map<String, Double> getTeamNoTreeMap(GrpInsurAppl grpInsurAppl) {
		...
		if(!"L".equals(grpInsurAppl.getLstProcType())){
			...
			if(PREM_SOURCE.GRP_ACC_PAY.getKey().equals(grpInsurAppl.getPaymentInfo().getPremSource())){ 
				//无组织架构树的被保人加费
				double addFeeAmnt=procMioInfoDao.getPersonAddMoney(grpInsurAppl.getApplNo(),null);
				sumDwJe+=addFeeAmnt;
			}
		}
		String key=applNo+"O0";
	 ...
	 * @param bigMap 账户信息
	 */
	private boolean createEaccData(String applNo, Map<String, Double> bigMap){
		this.logger.info("冻结帐号金额业务:【"+applNo+"】!");
		List<EarnestAccInfo> earList = new ArrayList<>();
		for(Entry<String,Double> enrtry:bigMap.entrySet()){
			String accNo = enrtry.getKey(); 	//帐号
			BigDecimal accJe = BigDecimal.valueOf(enrtry.getValue());	//应收金额
			//获取账户信息
			EarnestAccInfo earAcc = earnestAccInfoDao.queryEarnestAccInfo(accNo);
			if(earAcc == null ){
				logger.info("根据帐号未查询到对应的帐号信息【"+accNo+"】!");
				return false;
			}
			//余额减去应收金额
			earAcc.setBalance(earAcc.getBalance().subtract(accJe));
			//冻结金额加上应收金额
			earAcc.setFrozenBalance(earAcc.getFrozenBalance().add(accJe));
			earList.add(earAcc);
		}
		boolean isSuccess = earnestAccInfoDao.updateEarnestAccInfo(earList);
		return isSuccess;
	}
