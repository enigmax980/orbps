...
public class FaceAmntAndPremiumCheckTasket implements Tasklet{
	@Autowired
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		for(JSONObject jsonObject:jsonObjects){
			...
			if(polCode.length() > 3){
				...
				if(mapIpsnFaceAmntSub.containsKey(polCode)){
					mapIpsnFaceAmntSub.put(polCode, mapIpsnFaceAmntSub.get(polCode) + jsonObject.getDouble("sumFaceAmnt"));
				}else {
				...
				if(mapIpsnFaceAmnt.containsKey(polCode.substring(0,3))){
					mapIpsnFaceAmnt.put(polCode.substring(0,3), mapIpsnFaceAmnt.get(polCode.substring(0,3)) + jsonObject.getDouble("sumFaceAmnt"));
				}else {
				...
				if(mapIpsnPremium.containsKey(polCode.substring(0,3))){
					mapIpsnPremium.put(polCode.substring(0,3), mapIpsnPremium.get(polCode.substring(0,3)) + jsonObject.getDouble("sumPremium"));
				}else {
		...
		for(Map.Entry<String, Double> entry:mapFaceAmnt.entrySet()){ 
			...
			if(Math.abs(entry.getValue() - mapIpsnFaceAmnt.get(entry.getKey())) > 0.001){
				ecseptionStr.append("|险种["+entry.getKey()+"]总保额["+entry.getValue()+"]跟被保人险种累计总保额["+mapIpsnFaceAmnt.get(entry.getKey())+"]不一致|");
			}
		} 		
		for(Map.Entry<String, Double> entry:mapPremium.entrySet()){ 
			...
			if(Math.abs(entry.getValue() - mapIpsnPremium.get(entry.getKey())) > 0.001){
				ecseptionStr.append("|险种["+entry.getKey()+"]总保费"+entry.getValue()+"跟被保人险种累计总保费["+mapIpsnPremium.get(entry.getKey())+"]不一致|");
			}
		} 				
		/*对子险种保额进行校验*/
		...
		if(Math.abs(entry.getValue() - mapIpsnFaceAmntSub.get(entry.getKey())) > 0.001){
			ecseptionStr.append("|子险种["+entry.getKey()+"]总保额["+entry.getValue()+"]跟被保人子险种累计总保额["+mapIpsnFaceAmntSub.get(entry.getKey())+"]不一致|");
			}
		} 		
		for(Map.Entry<String, Double> entry:mapPremiumSub.entrySet()){ 
			...
			if(Math.abs(entry.getValue() - mapIpsnPremiumSub.get(entry.getKey())) > 0.001){
				ecseptionStr.append("|子险种["+entry.getKey()+"]总保费["+entry.getValue()+"]跟被保人子险种累计总保费["+mapIpsnPremiumSub.get(entry.getKey())+"]不一致|");
			}
		} 			
		if(Math.abs(SumIpsnFaceAmnt - sumFaceAmnt) >0.001){
			ecseptionStr.append("|保单要约总保额["+sumFaceAmnt+"]跟被保人累计总保额["+SumIpsnFaceAmnt+"]不一致|");
		}
		if(Math.abs(SumIpsnPremium - sumPremium) >0.001){
			ecseptionStr.append("|保单要约总保费["+sumPremium+"]跟被保人累计总保费["+SumIpsnPremium+"]不一致|");
		}
		...
		/*被保人清单 与 保单要约 进行校验*/		
		/*对被保总人数人数进行校验*/
		long ipsnNum = mongoTemplate.count(Query.query(Criteria.where("applNo").is(grpInsurAppl.getApplNo())), GrpInsured.class);
		if(ipsnNum != grpInsurAppl.getApplState().getIpsnNum()){
			ecseptionStr.append("|被保人清单中投保总人数["+ipsnNum+"]与投保要约中投保总人数["+grpInsurAppl.getApplState().getIpsnNum()+"]不一致!|");
		}
	...
	private void  checkOrgTrees(List<OrgTree> orgTrees,String applNo,StringBuilder errMsg){
		...
		/*组织架构树目前所采用校验规则
		 *1、所有节点 即可以是缴费节点又可以是非缴费节点   (不用校验)
		  2、不是所有节点都挂人    (不用校验)
		  3、如果有一个被保人挂在节点上，所有被保人都必须挂在节点上 (校验)
		  4、根节点如果挂人，就必须是缴费节点，其他节点不限制 (校验)
		  5、如果是缴费节点，必须挂人 (校验)
		  6、如果该节点是缴费节点，挂在该节点下的被保人及单位保费金额就保存在该节点上 (校验)
		...
		for(OrgTree orgTree:orgTrees){
			mapLevel.put(String.valueOf(orgTree.getLevelCode()),null == orgTree.getNodePayAmnt()?0.00:orgTree.getNodePayAmnt());
		...
		Aggregation aggregationOrg = Aggregation.newAggregation(
				Aggregation.match(Criteria.where("applNo").is(applNo)), 
				Aggregation.group("levelCode").sum("grpPayAmount").as("sumGrpPayAmount")
				.push("ipsnNo").as("ipsnNoList")
				);
		AggregationResults<JSONObject> aggregationResults = mongoTemplate.aggregate(aggregationOrg, GrpInsured.class, JSONObject.class);			
		...
		for(JSONObject jsonObject:mappedResults){
			mapStat.put(jsonObject.getString("_id"), null == jsonObject.getDouble("sumGrpPayAmount")?0.00:jsonObject.getDouble("sumGrpPayAmount"));
		}
		...
		for(JSONObject jsonObject:mappedResults){
			...
			if (mapIsPaid.containsKey(jsonObject.getString("_id"))) {
				...
				String levelCode[] = StringUtils.isEmpty(mapIsPaid.get(jsonObject.getString("_id")))?null:mapIsPaid.get(jsonObject.getString("_id")).split("\\|");
				...
				if(Math.abs(sum - mapStat.get(jsonObject.getString("_id"))) > 0.001) {
					errMsg.append("层级代码["+jsonObject.getString("_id")+"]为缴费节点,该节点所占金额["+sum+"]跟被保人清单中,该层级代码及其非缴费子节点累计单位交费金额["+mapStat.get(jsonObject.getString("_id"))+"]不一致|");
					continue;
				}
			}
			...
			if(mapNoPaid.containsKey(jsonObject.getString("_id"))){
				...
				String levelCode[] = StringUtils.isEmpty(mapIsPaid.get(orgTree.getLevelCode()))?null:mapIsPaid.get(orgTree.getLevelCode()).split("\\|");
				...
				if( Math.abs(surplus - mapStat.get(jsonObject.getString("_id")))> 0.0001 ){
					errMsg.append("层次代码["+jsonObject.getString("_id")+"]为非缴费节点,其在被保人清单中累计单位交费金额["+mapStat.get(jsonObject.getString("_id"))+"]跟组织架构树中该节点所占金额["+surplus+"]不一致|");
					continue;
				}
			}
		}		
		...
		for(Map.Entry<String, String> entry:mapPayType.entrySet()){
			if(StringUtils.equals("Y", entry.getValue())){
				...
				if(null == orgTree){
					...
					lMaps.put(orgTree.getLevelCode(), (StringUtils.isEmpty(lMaps.get(orgTree.getLevelCode()))?entry.getKey():(lMaps.get(orgTree.getLevelCode())+entry.getKey()))+"|");
				}else {
