...
public class TaskPrmyRowMapper implements RowMapper<TaskPrmyInfo>{
	...
	public TaskPrmyInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if (!StringUtils.isEmpty(rs.getString("CREATE_TIME"))) {
			taskPrmyInfo.setCreateTime(DateFormatUtils.format(rs.getTimestamp("CREATE_TIME"), pattern));
		}
		if (!StringUtils.isEmpty(rs.getString("START_TIME"))) {
			taskPrmyInfo.setStartTime(DateFormatUtils.format(rs.getTimestamp("START_TIME"), pattern));
		}
		if (!StringUtils.isEmpty(rs.getString("END_TIME"))) {
			taskPrmyInfo.setEndTime(DateFormatUtils.format(rs.getTimestamp("END_TIME"), pattern));
		}
		...
		taskPrmyInfo.setExtKey5(rs.getString("EXT_KEY5"));
