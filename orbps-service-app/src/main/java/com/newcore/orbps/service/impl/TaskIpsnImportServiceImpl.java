...
import com.newcore.orbps.service.api.TaskIpsnImportService;
import com.newcore.tsms.service.api.task.TaskProcessService;

/**
 * @ClassName: TaskIpsnImportServiceImpl
 * @Description: 清单导入
 * @author jiangbomeng
 * @date 2016年9月1日 上午11:06:50
 * 
 */
@Service("taskIpsnImportService")
public class TaskIpsnImportServiceImpl implements TaskIpsnImportService {
	/**
	 * 日志管理工具实例.
	 */
	private static Logger logger = LoggerFactory.getLogger(TaskIpsnImportServiceImpl.class);
	...
	@Override
	@AsynCall
	public String importIpsnList(String args) {
		String[] argsArr = args.split("_");

		// 业务逻辑开始
		logger.info("执行方法:清单导入" + argsArr[0] + "_" + argsArr[1]);
