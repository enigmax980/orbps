...
public class ProcMioInfoDaoImpl implements ProcMioInfoDao{
	...
	@Override
	public double getPersonAddMoney(String applNo,String ipsnId) {
		Aggregation agg ;
		if("".equals(ipsnId)){
			agg = Aggregation.newAggregation(
			...
			String [] ipsnIds = ipsnId.split(",");
			for(int i=0;i<ipsnIds.length;i++){
				values.add(Long.parseLong(ipsnIds[i]));
			}
			agg = Aggregation.newAggregation(
					Aggregation.match(Criteria.where("businessKey").is(applNo)),
					Aggregation.unwind("udwIpsnResults"),				
					Aggregation.unwind("udwIpsnResults.udwIpsnPolResults"),
					Aggregation.match(Criteria.where("udwIpsnResults.ipsnNo").in(values)),
					Aggregation.group("sumJe").sum("udwIpsnResults.udwIpsnPolResults.addFeeAmnt").as("sumAddFeeAmnt")
					);
		}
	...
	public double getPersonAddMoney(String applNo, String polCode, long ipsnNo) {
		Aggregation agg = Aggregation.newAggregation(
				...
				Aggregation.unwind("udwIpsnResults.udwIpsnPolResults"),
				Aggregation.match(Criteria.where("udwIpsnPolResults.polCode").is(polCode)),
				Aggregation.group("sumJe").sum("udwIpsnPolResults.addFeeAmnt").as("sumAddFeeAmnt")
				);
		AggregationResults<Object> resuAgg = mongoTemplate.aggregate(agg,"applUdwResult", Object.class);
		BasicDBList dbdList = (BasicDBList) resuAgg.getRawResults().get("result");
		if(dbdList.isEmpty()){
	...
	@Override
	public int changeGrpInsuredState(PlnmioRec plnmioRec, String procStat) {
		...
		if("O".equals(multiPayAccType)){
			grpInsMap.put("levelCode", plnmioRec.getLevelCode());
		...
		List<GrpInsured> grpInsuredList = (List<GrpInsured>) mongoBaseDao.find(GrpInsured.class, grpInsMap);
		int rows =0;
		for(GrpInsured grpInsured:grpInsuredList){
			Map<String,Object> ipsnIDMap = new HashMap<>();
			ipsnIDMap.put("applNo", applNo);
			ipsnIDMap.put("ipsnNo", Long.valueOf(grpInsured.getIpsnNo()));
			...
			if("O".equals(procStat)){
				...
				gupInsUpdate.set("remark",grpInsured.getRemark()+"扣款超过截止日期，扣款失败!");
			}else if("E".equals(procStat)){
				gupInsUpdate.set("remark","完成实收");
			}
			rows+=mongoBaseDao.update(GrpInsured.class, ipsnIDMap, gupInsUpdate);  
		}
		return rows;
	}
