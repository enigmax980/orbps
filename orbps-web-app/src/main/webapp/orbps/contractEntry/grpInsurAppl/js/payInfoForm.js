com.orbps.contractEntry.grpInsurAppl.payInfoForm = $("#payInfoForm");
com.orbps.contractEntry.grpInsurAppl.settleList = [];
var numflag = 0;//用于记录change事件的次数。
// 基本信息校验规则
com.orbps.contractEntry.grpInsurAppl.payValidateForm = function(vform) {
...
$(function() {
	...
	com.orbps.contractEntry.grpInsurAppl
			.payValidateForm(com.orbps.contractEntry.grpInsurAppl.payInfoForm);
		...
		.click(
				function() {
					...
					com.orbps.contractEntry.grpInsurAppl.addDialog
							.load(
									"/orbps/orbps/contractEntry/grpInsurAppl/html/settlementDateModal.html",
									function() {
										$(this).modal('toggle');
									});
					setTimeout(function() {
...
// 重新加载表格
com.orbps.contractEntry.grpInsurAppl.reloadsettlementInsuredModalTable = function() {
	$('#settlementDataGrid').find("tbody").empty();
	if (com.orbps.contractEntry.grpInsurAppl.settleList != null
			&& com.orbps.contractEntry.grpInsurAppl.settleList.length > 0) {
		for (var i = 0; i < com.orbps.contractEntry.grpInsurAppl.settleList.length; i++) {
			settlementDatas = new Date(
					com.orbps.contractEntry.grpInsurAppl.settleList[i])
					.format("yyyy-MM-dd");
			$('#settlementDataGrid')
					.find("tbody")
					.append(
							"<tr role='row' class='odd' id='settlementDataGridtr0' data-type='add' data-updating='false'><td ><input type='radio' onclick='com.orbps.contractEntry.grpInsurAppl.dateRadio();' id='insuredRad"
									+ i
									+ "' name='insuredRad' value='"
									+ i
									+ "'></td><td >"
									+ settlementDatas
									+ "</td><td ><a href='javascript:void(0);' onclick='com.orbps.contractEntry.grpInsurAppl.deleteSettleDate("
									+ i
