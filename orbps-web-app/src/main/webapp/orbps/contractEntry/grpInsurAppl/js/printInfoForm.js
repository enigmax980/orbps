com.orbps.contractEntry.grpInsurAppl.printInfoForm = $("#printInfoForm");
// 基本信息校验规则
com.orbps.contractEntry.grpInsurAppl.printValidateForm = function(vform) {
...
$(function() {
	...
	com.orbps.contractEntry.grpInsurAppl
			.printValidateForm(com.orbps.contractEntry.grpInsurAppl.printInfoForm);
});
...
$("#printInfoForm #ipsnlstId").change(
		function() {
			...
			$("#prtIpsnLstType").attr("disabled", false);
			$("#ipsnVoucherPrt").attr("disabled", false);
			//当页面选择无清单时清单打印置灰，
			if ($("#printInfoForm #ipsnlstId").val() === "N") {
				...
				$("#prtIpsnLstType").attr("disabled", true);
				$("#ipsnVoucherPrt").attr("disabled", true);
			} else {
				// 展示必录的红*
