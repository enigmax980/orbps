com.orbps.contractEntry.sgGrpInsurAppl.personalInsurInfoForm = $("#personalInsurInfoForm");

com.orbps.contractEntry.sgGrpInsurAppl.personBaseValidateForm = function(vform) {
...
$(function() {
	//保额键盘抬起事件
	$('body').delegate('table#bsInfoListTb #amount' , 'keyup', function(){
		if(commevent(this)){
			return;
		}
	...
	// 保费键盘抬起事件
	$('body').delegate('table#bsInfoListTb #premium' , 'keyup', function(){
		if(commevent(this)){
			return;
		}
		if(!isNaN(this.value)){this.value=/^\d+\.?\d{0,2}$/.test(this.value) ? this.value : this.value.substring(0,this.value.length-1)}else{this.value = ''};
	...
	$('body').delegate('table#bsInfoListTb #premium' , 'blur', function(){
	 ...
	 //保险期间键盘抬起事件
	$('body').delegate('table#bsInfoListTb #validateDate' , 'keyup', function(){
		if(commevent(this)){
			return;
		}
		if(this.value.length==1){if(this.value<0){this.value=''}this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')};
		$("#validateDate").val(this.value);
	});
	
	 // 承保人数键盘抬起事件
	$('body').delegate('table#bsInfoListTb #insuredNum' , 'keyup', function(){
		if(commevent(this)){
			return;
		}
		if(this.value.length==1){if(this.value<1){this.value=''}this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')};
		$("#insuredNum").val(this.value);
	});
	
	// 承保人数丢失焦点事件
	$('body').delegate('table#bsInfoListTb #insuredNum' , 'blur', function(){
		if(this.value.length==1){if(this.value<1){this.value=''}this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')};
		$("#insuredNum").val(this.value);
	});
    ...
    com.orbps.contractEntry.sgGrpInsurAppl
            .personBaseValidateForm(com.orbps.contractEntry.sgGrpInsurAppl.personalInsurInfoForm);
 ...
 // 汇交人身份证号验证
  ...
  $("#joinIdNo").blur(
    ...
    $("#joinName").blur(function() {
     ...
     // 汇交人姓名只能有一个空格。
        joinName=joinName.replace(/^ +| +$/g,'').replace(/ +/g,' ');
    	$("#joinName").val(joinName);
    });
    ...
    $("#postCode").blur(function() {
        ...
        if (postCode === null || "" === postCode) {
            lion.util.info("警告", "汇交人邮政编码不能为空");
            return false;
        }
    });
    $("#personalInsurInfoForm #joinHome").blur(function() {
        var home = $("#personalInsurInfoForm #joinHome").val();
        if (home === null || "" === home) {
...
$("#personalInsurInfoForm #joinIdType").change(function() {
	var joinIdType = $("#personalInsurInfoForm #joinIdType").val();
	  if(joinIdType === "I"){
		...
		//页面回显会有两次的change事件，
	  if(com.orbps.contractEntry.sgGrpInsurAppl.acount === 1 || com.orbps.contractEntry.sgGrpInsurAppl.acount ===2){
		  com.orbps.contractEntry.sgGrpInsurAppl.acount++;
		  return false;
	  }
